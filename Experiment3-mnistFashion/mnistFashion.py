...
from tensorflow.keras.datasets import fashion_mnist  # Changed from mnist to fashion_mnist

# Parse command-line arguments
model_name = sys.argv[1]
target_category = int(sys.argv[2])
generation_interval = int(sys.argv[3])
replicate = int(sys.argv[4])
...
(X_train, y_train), (X_test, y_test) = fashion_mnist.load_data()  # Changed dataset
...
def evaluate_model(individual, model, input_shape, target_category):
    ...
    if isinstance(model, tf.keras.Model):
        if len(input_shape) == 2:
            image = np.array(individual).reshape(1, *input_shape, 1)
        else:
